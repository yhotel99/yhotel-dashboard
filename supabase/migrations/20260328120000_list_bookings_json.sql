-- Single RPC: paginated rows + total count
-- p_status: NULL / '' = mọi trạng thái
-- Keyset: truyền cả p_cursor_created_at + p_cursor_id → bỏ OFFSET (trang tiếp theo)
-- Offset: để cursor NULL, dùng p_page + p_limit như cũ
-- matching: DISTINCT b.id (join booking_rooms/rooms chỉ phục vụ lọc search, không join lại ở page_booking)

DROP FUNCTION IF EXISTS public.list_bookings_json(text, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.list_bookings_json(text, integer, integer, uuid, text);
