-- list_bookings_json v2: EXISTS search (no DISTINCT/materialize), p_include_total, limit+1 cursor
-- Indexes for list query filters
-- NOTE: LANGUAGE plpgsql — OFFSET/LIMIT must use plpgsql variables only (not CTE columns)

SET search_path = public;

CREATE INDEX IF NOT EXISTS idx_bookings_branch_check_in
  ON public.bookings (branch_id, check_in DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_payments_branch_paid_at
  ON public.payments (branch_id, paid_at DESC)
  WHERE paid_at IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_bookings_branch_created_at_id
  ON public.bookings (branch_id, created_at DESC, id DESC)
  WHERE deleted_at IS NULL;
...
BEGIN
  ...
  IF p_date_field IN ('created_at', 'check_in') THEN
  ...
  WITH page_raw AS (
    ...
    WHERE b.deleted_at IS NULL
      ...
      AND (
        ...
        OR (
          CASE
            WHEN v_normalized_date_field = 'check_in' THEN (b.check_in AT TIME ZONE 'UTC')::date
