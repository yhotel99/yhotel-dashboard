...
-- Fix: Bỏ điều kiện br.check_in <= now() + interval '2 hours' trong join
--      để booking tương lai xa (19/3, 22/3...) đều lọt vào CTE,
--      row_number sẽ chọn booking check_in GẦN nhất.
-- ============================================================================
...
WITH room_active_booking AS (
            ...
            ORDER BY
                CASE
                    ...
                    WHEN b.status = 'checked_in'
                        AND now() >= br.check_out
                    THEN 2
                    -- Sắp nhận trong vòng 2 giờ tới
                    WHEN b.status IN ('confirmed','awaiting_payment','pending')
                        AND now() >= (br.check_in - interval '2 hours')
                        AND now() < br.check_in
                    THEN 3
                    -- Booking tương lai xa → vẫn lấy gần nhất nhờ check_in ASC
                    WHEN b.status IN ('confirmed','awaiting_payment','pending')
                        AND now() < (br.check_in - interval '2 hours')
                    THEN 4
                    ELSE 99
                END ASC,
                br.check_in ASC  -- cùng priority → check_in gần nhất lên đầu
        ) AS rn
    ...
    LEFT JOIN public.bookings b
        ...
        AND (
            br.check_out > now()        -- booking chưa kết thúc (cover toàn bộ tương lai)
            OR b.status = 'checked_in'  -- hoặc đang ở (phòng overdue checkout)
        )
)
SELECT
    r.id,
