-- Add created_by to bookings for KPI reporting by user/profile
-- - Store auth.uid() (maps to profiles.id in this project)
-- - Keep nullable for legacy data / service role inserts
...
-- 2) Indexes for reports
CREATE INDEX IF NOT EXISTS idx_bookings_created_by_created_at
ON public.bookings(created_by, created_at DESC);

-- 3) Update RPCs to set created_by = auth.uid()
--    Recreate from latest definitions (voucher + final_amount support)
