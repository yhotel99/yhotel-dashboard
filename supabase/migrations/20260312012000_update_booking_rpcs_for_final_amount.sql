-- Migration: Update booking RPCs to support final_amount
-- - Extend create_booking_secure & create_multi_booking_secure
-- - Store both total_amount (giá gốc) và final_amount (thanh toán cuối)
-- - Payments (advance_payment, room_charge) dựa trên final_amount

DROP FUNCTION IF EXISTS public.create_booking_secure(
  uuid, uuid, timestamptz, timestamptz, integer, numeric, text, integer, text, numeric
) CASCADE;
...
-- Multi-room booking: thêm p_final_amount, payments dựa trên final_amount
DROP FUNCTION IF EXISTS public.create_multi_booking_secure(
  uuid, jsonb, timestamptz, timestamptz, integer, integer, text, text, numeric
) CASCADE;
