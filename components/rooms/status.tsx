...
export function StatusBadge({ status }: { status: Room["status"] | string }) {
  ...
  > = {
    [ROOM_STATUS.AVAILABLE]: { label: roomStatusLabels[ROOM_STATUS.AVAILABLE], variant: "default" },
    [ROOM_STATUS.MAINTENANCE]: { label: roomStatusLabels[ROOM_STATUS.MAINTENANCE], variant: "outline" },
    [ROOM_STATUS.NOT_CLEAN]: { label: roomStatusLabels[ROOM_STATUS.NOT_CLEAN], variant: "destructive" },
    [ROOM_STATUS.CLEAN]: {
  ...
  const config =
    statusConfig[status as Room["status"]] ||
    defaultConfig;
